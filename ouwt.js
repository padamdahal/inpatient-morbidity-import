...
$(function() {
  ...
  if (dhis2.ou.memoryOnly) {
    ...
    adapters = [
      dhis2.storage.IndexedDBAdapter,
      dhis2.storage.DomLocalStorageAdapter,
      dhis2.storage.InMemoryAdapter
    ];
    partial_adapters = [
      dhis2.storage.IndexedDBAdapter,
      dhis2.storage.DomSessionStorageAdapter,
      dhis2.storage.InMemoryAdapter
    ];
  }
...
function Selection() {
  ...
  this.ajaxOrganisationUnits = function(versionOnly, format) {
    ...
    return $.ajax({
      url:
        dhis2BaseUrl +
        "/dhis-web-commons-ajax-json/getOrganisationUnitTree.action",
      data: {
  ...
  this.load = function() {
    ...
    selection
      .ajaxOrganisationUnits(true, "text")
      .done(function(data) {
  ...
  this.clear = function() {
    ...
    var roots = selection.getRoots();
    selection.getRoots().length > 1
      ? selection.setSelected(roots)
      : selection.setSelected(roots[0]);
    subtree.reloadTree();

    $.post(organisationUnitTreePath + "clearselected.action").complete(
  ...
  this.select = function(unitId) {
    ...
    var $linkTag = $("#" + getTagId(unitId))
      .find("a")
      .eq(0);

    if (
      $linkTag.hasClass("selected") &&
      (unselectAllowed || rootUnselectAllowed)
    ) {
      var selected = selection.getSelected();
  ...
  this.responseReceived = function() {
    ...
    if (multipleSelectionAllowed) {
      ...
      $(document).trigger(dhis2.ou.event.orgUnitSelected, [
        ids,
        names,
        children
      ]);
  ...
  this.findByName = function() {
    ...
    if (match !== undefined) {
      ...
      multipleSelectionAllowed
        ? selection.setSelected([match.id])
        : selection.setSelected(match.id);
...
function Subtree() {
  this.ajaxGetChildren = function(parentId) {
    return $.post(
      dhis2BaseUrl +
        "/dhis-web-commons-ajax-json/getOrganisationUnitTree.action?parentId=" +
        parentId
    );
  };
  ...
  this.ajaxGetLeaf = function(parentId) {
    return $.post(
      dhis2BaseUrl +
        "/dhis-web-commons-ajax-json/getOrganisationUnitTree.action?leafId=" +
        parentId
    );
  };
  ...
  function createTreeElementTag(ou) {
    ...
    if (ou.c.length > 0) {
      $toggleTag.bind(
        "click",
        new Function('subtree.toggle( "' + ou.id + '" )')
      );
    ...
    $linkTag.attr(
      "href",
      'javascript:void selection.select( "' + ou.id + '" )'
    );
    $linkTag.append(ou.n);
  ...
  function getToggleExpand() {
    return getToggleImage()
      .attr("src", dhis2BaseUrl + "/images/colapse.png")
      .attr("alt", "[+]");
  }

  function getToggleCollapse() {
    return getToggleImage()
      .attr("src", dhis2BaseUrl + "/images/expand.png")
      .attr("alt", "[-]");
  }

  function getToggleBlank() {
    return getToggleImage()
      .attr("src", dhis2BaseUrl + "/images/transparent.gif")
      .removeAttr("alt");
  }
